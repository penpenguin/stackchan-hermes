// Derived from locked espressif/esp_video 1.3.1. Added bounded capture dequeue.
/*
...
static esp_err_t esp_video_ioctl_dqbuf(struct esp_video *video, struct v4l2_buffer *vbuf, uint32_t ticks)
{
    esp_err_t ret;
    struct esp_video_buffer_info info;
...
esp_err_t esp_video_ioctl(struct esp_video *video, int cmd, va_list args)
{
    ...
    case VIDIOC_QBUF:
        ...
        break;
    case VIDIOC_CAPTURE_DQBUF: {
        struct stackchan_capture_buffer *request = (struct stackchan_capture_buffer *)arg_ptr;
        ret = esp_video_ioctl_dqbuf(video, &request->buffer, request->ticks);
        break;
    }
    case VIDIOC_DQBUF:
        ret = esp_video_ioctl_dqbuf(video, (struct v4l2_buffer *)arg_ptr, portMAX_DELAY);
        break;
