...
// Derived from 78/esp-ml307 3.6.5 src/esp/esp_tcp.cc.
// Changes: reject missing event groups/receive tasks, clean failed connections, reject zero sends.
#include <cerrno>
#include "esp_tcp.h"
...
bool EspTcp::Connect(const std::string& host, int port) {
    if (!event_group_) { last_error_ = ENOMEM; return false; }
    // 确保先断开已有连接
    if (connected_) {
        Disconnect();
    }
    ...
    if (task_created != pdPASS) {
        ...
        ESP_LOGE(TAG, "Receive task creation failed: error=%d", last_error_);
        return false;
    }
    return true;
}
