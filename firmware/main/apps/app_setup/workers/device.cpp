...
static std::string _tag = "Setup-Device";
...
DevicePowerSavingWorker::DevicePowerSavingWorker()
{
    mclog::info("DevicePowerSavingWorker start");

    for (uint32_t seconds = 0; seconds <= 3600; seconds += 300) {
        _idle_shutdown_levels.push_back(seconds);
    }

    _config = GetHAL().getDeviceConfig();
...
void DevicePowerSavingWorker::update()
{
    ...
    if (_confirm_flag) {
        ...
        GetHAL().setDeviceConfig(_config);
...
void DevicePowerSavingWorker::update_idle_label()
{
...
DeviceMovementWorker::DeviceMovementWorker()
{
    mclog::info("DeviceMovementWorker start");
...
void DeviceMovementWorker::update()
{
...
void DeviceMovementWorker::update_idle_motion_label()
{
