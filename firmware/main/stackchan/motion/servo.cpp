...
{
    if (!allowsMotionCommand(MotionCommandSource::Local)) {
        return;
    }
    apply_default_spring_options();
...
void Servo::moveWithSpringParams(int angle, float stiffness, float damping)
{
    ...
    move_with_spring_params_impl(angle, stiffness, damping);
}
...
void Servo::moveWithSpeed(int angle, int speed)
{
    ...
    move_with_speed_impl(angle, speed);
}

void Servo::moveFromAuthenticatedBridgeWithSpeed(int angle, int speed)
{
    if (!allowsMotionCommand(MotionCommandSource::AuthenticatedBridge)) {
        return;
    }
    move_with_speed_impl(angle, speed);
}

void Servo::setLocalMotionLocked(bool locked)
{
    _command_safety.setLocalMotionLocked(locked);
}

bool Servo::localMotionLocked() const
{
    return _command_safety.localMotionLocked();
}

bool Servo::allowsMotionCommand(MotionCommandSource source) const
{
    return _command_safety.allowsMotion(source);
}

bool Servo::allowsTorqueCommand(bool enabled, MotionCommandSource source) const
{
    return _command_safety.allowsTorque(enabled, source);
}

void Servo::move_with_spring_params_impl(int angle, float stiffness, float damping)
{
...
void Servo::move_with_speed_impl(int angle, int speed)
{
    auto spring_options = map_speed_to_spring_options(speed);
    move_with_spring_params_impl(angle, spring_options.stiffness, spring_options.damping);
}
...
{
    speed = stackchan::bridge_client::K151MotionSafety::clampDriverSpeed(speed);
