...
bool StackChanAvatarDisplay::SetBlinkEnabled(bool enabled)
{
    auto& stackchan = GetStackChan();
    if (!stackchan.hasAvatar()) {
        return false;
    ...
    DisplayLockGuard lock(this);
    if (enabled && blink_modifier_id_ < 0) {
        blink_modifier_id_ = stackchan.addModifier(std::make_unique<BlinkModifier>());
    } else if (!enabled && blink_modifier_id_ >= 0) {
...
void StackChanAvatarDisplay::SetStatus(const char* status)
{
    ...
    if (strcmp(status, Lang::Strings::SPEAKING) != 0 && speaking_modifier_id_ >= 0) {
...
{
    if (notification == nullptr || duration_ms <= 0) {
        return;
    }
    view::pop_a_toast(notification, view::ToastType::Info, duration_ms);
}
