...
static constexpr std::uint8_t _stable_press_samples = 6;
static constexpr std::uint8_t _stable_release_samples = 3;
...
public:
    ...
    HeadPetGesture update(const TouchData& data)
    {
        ...
        const bool raw_touched = data.is_touched(config.touch_threshold);
        const HeadTouchTransition transition = debouncer.update(raw_touched);

        switch (current_state) {
            case TouchState::IDLE:
                if (transition == HeadTouchTransition::Pressed) {
            ...
            case TouchState::TOUCHED:
                if (transition == HeadTouchTransition::Released) {
                    current_state = TouchState::IDLE;
                    gesture       = HeadPetGesture::Release;
                } else if (raw_touched && debouncer.isTouched()) {
                    // Check for swipe
...
private:
    TouchConfig config;
    HeadTouchDebouncer debouncer{_stable_press_samples, _stable_release_samples};
