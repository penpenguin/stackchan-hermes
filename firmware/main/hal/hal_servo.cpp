...
static constexpr bool _head_motion_locked = true;
#else
static constexpr bool _head_motion_locked = false;
#endif
...
public:
    ...
    {
        if (!allowsTorqueCommand(enabled, MotionCommandSource::Local)) {
            return;
        }
        Servo::setTorqueEnabled(enabled);
        _output_safety.writeTorque(enabled, [&]() { _scs_bus.EnableTorque(_config.id, enabled ? 1 : 0); });
...
private:
    ...
    const ServoOutputSafety _output_safety {_head_motion_locked};
...
void Hal::servo_init()
{
    ...
    yaw_servo_config.angleLimit             = Vector2i(-450, 450);
...
#if CONFIG_STACKCHAN_HERMES_LOCAL_MOTION_LOCK
    motion->setLocalMotionLocked(true);
#endif
    motion->init();
